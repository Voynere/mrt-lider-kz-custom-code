...
$known_city_slugs_booking_form_php = array(
...
if (!function_exists('get_city_slug_from_request_path_booking_form')) {
    function get_city_slug_from_request_path_booking_form($known_slugs) {
...
$city_slug_from_url_booking_form = get_city_slug_from_request_path_booking_form($known_city_slugs_booking_form_php);

if ($city_slug_from_url_booking_form !== false) {
    $selected_city_booking_form = $city_slug_from_url_booking_form;
    if (!headers_sent()) {
        if (!isset($_COOKIE['selected_city']) || $_COOKIE['selected_city'] !== $selected_city_booking_form) {
            setcookie('selected_city', $selected_city_booking_form, time() + 30 * DAY_IN_SECONDS, '/', $_SERVER['HTTP_HOST'], is_ssl(), true);
        }
    }
} else {
    $selected_city_cookie_value_booking_form = isset($_COOKIE['selected_city']) ? sanitize_text_field($_COOKIE['selected_city']) : 'tumen';
    if (in_array($selected_city_cookie_value_booking_form, $known_city_slugs_booking_form_php, true)) {
         $selected_city_booking_form = $selected_city_cookie_value_booking_form;
    } else {
         $selected_city_booking_form = 'tumen';
         if (!headers_sent()) {
            setcookie('selected_city', $selected_city_booking_form, time() + 30 * DAY_IN_SECONDS, '/', $_SERVER['HTTP_HOST'], is_ssl(), true);
         }
    }
...
// Используем переопределенный $selected_city_booking_form вместо оригинального $selected_city
$args = array(
    'post_type'      => 'post',
    'posts_per_page' => 1,
    'tax_query'      => array(
        ...
        array(
            'taxonomy' => 'category',
            'field' => 'slug',
            'terms' => $selected_city_booking_form, // Используем переопределенный $selected_city_booking_form
        ),
        array(
            'taxonomy' => 'category',
            'field' => 'slug',
...
// Тема письма
$email_subject = 'Новая запись из модального окна';
